...
function mockProject() {
    ...
    if (!fs.existsSync(path.join(require("os").homedir() + '/.pm-tool/projektmanagementtool.json'))) {
        fs.writeFileSync(path.join(require("os").homedir() + '/.pm-tool/') + testProject.title.replace(/\s+/g, '').toLowerCase() + '.json', json, 'utf-8');
...
function loadProjects() {
    ...
    fs.readdirSync(path.join(require('os').homedir() + '/.pm-tool/')).filter(fn => fn.endsWith('.json')).forEach(function (elem, idx) {

        // Load files from disk and load into global variable
        projects.push(JSON.parse(fs.readFileSync(path.join(require('os').homedir() + '/.pm-tool/') + elem)));
